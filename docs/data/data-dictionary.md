...
Property matching treats naive database `effective_at` values as UTC before freshness comparison. Address normalization parses the final five-digit token as a postal code, so a leading street number such as `11704` remains a house number. A match run may use only the provider's current parcel projection; historical import IDs are rejected for new runs until a historical projection is implemented. The UI exposes acquisition mode, authorization basis, content-hash prefix, source version, and accepted-row count for the current snapshot.
