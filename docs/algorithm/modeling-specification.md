...
The Phase 5 baseline is an explicitly versioned weighted geometric mean over source quality, incident validity, property-match quality, material-loss evidence, loss complexity, Beyond Adjusting fit, and data sufficiency, with a freshness feature recorded separately. It is called a provisional ranking/evidence tier, not an empirical probability. Alerts must abstain when critical evidence is missing, a property match is unresolved, negative/contradictory evidence remains, or the source gate is not an authorized manual snapshot. The current `opportunity-scoring.v4` release records `beyondadjusting-fit.v2`, retrieved from the public [Beyond Adjusting service profile](https://beyondadjusting.com/#claim) on 2026-08-03. Its deterministic fit rule recognizes the published residential, commercial, condominium, and business property segments and gives a full fit signal only when the source taxonomy also identifies a fire-related event; property-only matches are partial, and absent or unmapped property segments remain missing/partial. For Sarasota property rows, code `0100` is normalized to the residential segment using the provider's published property-use field, with that mapping carried in the feature evidence. Repeated unchanged source captures are grouped into one evidence event for v4 scoring; raw source-observation IDs and retained-row counts remain in the score explanation for auditability. This is a service-profile comparison only: it does not infer damage, coverage, claim validity, willingness to hire, or whether a claim is new, underpaid, or denied. Every score carries an as-of boundary, feature versions, available-at timestamps, source observations, transformations, and explanation. Human overrides are append-only and cannot rewrite the baseline.
