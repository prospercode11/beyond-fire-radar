...
type SystemHealth = {
  ...
  live_polling_worker_enabled: boolean;
  live_polling_interval_seconds: number;
};
...
function Workspace({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    try {
      const [providerResult, incidentResult, opportunityResult, systemHealthResult] = await Promise.all([
